...
int main(void)
{
	...
	while (1) {
		...
		if ((vcom_connected() != 0) && (prompt == 0)) {
			vcom_write("Hello, this is a USB UPPER-to-", 30);
			__WFI(); // Wait for interupt, so endpoint buffer can be send.
			__WFI();
			__WFI();
			__WFI();
			__WFI();
			__WFI();
			__WFI();
			vcom_write("lower, and lower-to-UPPER char", 30);
			__WFI();
			__WFI();
			__WFI();
			__WFI();
